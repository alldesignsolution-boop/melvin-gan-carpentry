...
async def lifespan(app: FastAPI):
    ...
    # Seed default viewer account if it doesn't exist
    try:
        ...
        _db = SessionLocal()
        try:
            _VIEWER_PW = "Hoga-c7IcwQsgVrDW"
            _v = _db.query(UserModel).filter(UserModel.username == "viewer").first()
            if not _v:
                _db.add(UserModel(
                    username="viewer",
                    password_hash=hash_pw(_VIEWER_PW),
                    full_name="View Only",
                    role="viewer",
                    is_active=True,
                ))
                _db.commit()
                print("Created viewer account: username=viewer")
            elif _v.password_hash != hash_pw(_VIEWER_PW):
                # self-correct the viewer password to the current value
                # (sha256 is deterministic, so this hash compare is exact/idempotent)
                _v.password_hash = hash_pw(_VIEWER_PW)
                _db.commit()
                print("Updated viewer account password")
        finally:
            _db.close()
    except Exception as e:
        print(f"Viewer seed skipped: {e}")
    # Seed the Mr_Kiew admin account if it doesn't exist
    try:
        from database import SessionLocal
        from models import User as UserModel
        from auth import hash_pw
        _db = SessionLocal()
        try:
            if not _db.query(UserModel).filter(UserModel.username == "Mr_Kiew").first():
                _db.add(UserModel(
                    username="Mr_Kiew",
                    password_hash=hash_pw("Kiew@2026"),
                    full_name="Mr Kiew",
                    role="admin",
                    is_active=True,
                ))
                _db.commit()
                print("Created Mr_Kiew admin account")
        finally:
            _db.close()
    except Exception as e:
        print(f"Mr_Kiew seed skipped: {e}")
    # Seed the Mr_Calvin admin account if it doesn't exist (same password as Mr_Kiew)
    try:
        from database import SessionLocal
        from models import User as UserModel
        from auth import hash_pw
        _db = SessionLocal()
        try:
            _calvin = _db.query(UserModel).filter(UserModel.username == "Mr_Calvin").first()
            if not _calvin:
                _db.add(UserModel(
                    username="Mr_Calvin",
                    password_hash=hash_pw("Calvin@2026"),
                    full_name="Mr Calvin",
                    role="admin",
                    is_active=True,
                ))
                _db.commit()
                print("Created Mr_Calvin admin account")
            elif _calvin.password_hash != hash_pw("Calvin@2026"):
                # Correct the password if an earlier deploy seeded the wrong one
                _calvin.password_hash = hash_pw("Calvin@2026")
                _db.commit()
                print("Updated Mr_Calvin password")
        finally:
            _db.close()
    except Exception as e:
        print(f"Mr_Calvin seed skipped: {e}")
    # Retire the old "boss" admin login — it is now "melvin" / hoga@2026.
    # Idempotent: once boss is inactive and melvin is an active admin this is a no-op.
    try:
        from database import SessionLocal
        from models import User as UserModel, Lead as LeadModel
        from auth import hash_pw
        _db = SessionLocal()
        try:
            _boss = _db.query(UserModel).filter(UserModel.username == "boss").first()
            if _boss and _boss.is_active:
                # Anything still assigned to boss belongs to melvin now.
                _moved = _db.query(LeadModel).filter(LeadModel.assigned_to == "boss").update(
                    {"assigned_to": "melvin"}, synchronize_session=False)
                _boss.is_active = False
                _mel = _db.query(UserModel).filter(UserModel.username == "melvin").first()
                if _mel:
                    _mel.password_hash = hash_pw("hoga@2026")
                    _mel.full_name = "Melvin"
                    _mel.role = "admin"
                    _mel.is_active = True
                else:
                    _db.add(UserModel(
                        username="melvin",
                        password_hash=hash_pw("hoga@2026"),
                        full_name="Melvin",
                        role="admin",
                        is_active=True,
                    ))
                _db.commit()
                print(f"Migration: retired 'boss' login, melvin is now admin "
                      f"({_moved} lead(s) reassigned)")
        finally:
            _db.close()
    except Exception as e:
        print(f"Boss->melvin migration skipped: {e}")
